 ...
 * Props for `SwipeCard`.
 * @since 1.0.0
 * Position metadata (`index`/`total`) drives the "1 of N" progress indicator on Celo MiniPay.
 */
interface Props {
  task: TaskData;
...
export default function SwipeCard({ task, index, total }: Props) {
   ...
   * @since 1.0.0
   * Must stay in sync with the `AgentHands.sol` enum order — reordering silently breaks badge colors.
   */
